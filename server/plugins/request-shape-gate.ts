...
import { internalLlmRequestAuthorized } from '../agent-runs/internal-llm-auth.ts';
...
export function requestShapeAllowed(req: IncomingMessage): boolean {
  ...
  if (platformManaged()) {
    if (url.pathname === '/api/platform/session/exchange') return true;
    if (platformSession(req) && sameOriginRequest(req)) return true;
  }
