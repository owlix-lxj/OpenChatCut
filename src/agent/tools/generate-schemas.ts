...
export const GENERATE_TOOL_SCHEMAS: AgentToolSchema[] = [
  ...
  {
    name: 'submit_voice',
    description: 'Generate one TTS audio asset with ElevenLabs, Doubao, MiniMax, Inworld, Fish Audio, Speechify, OpenAI, Gemini, Mistral, Cartesia, or Alibaba Cloud Qwen-Audio. Creates a media-pool asset only; it does not place or replace timeline items. Confirm a configured provider and concrete provider-specific voice; MiniMax timbreWeights may mix voices with an empty voiceId.',
    input_schema: {
      type: 'object',
      properties: {
        provider: { type: 'string', enum: ['elevenlabs', 'doubao', 'minimax', 'inworld', 'fishaudio', 'speechify', 'openai', 'gemini', 'mistral', 'cartesia', 'qwen'], description: 'Required opt-in provider. Use only a provider listed as configured in capabilities. Voice IDs and catalogs are never interchangeable.' },
        ...
        modelId: { type: 'string', description: 'ElevenLabs, Inworld, Fish Audio, Speechify, OpenAI, Gemini, Mistral, Cartesia, or Qwen only. Omit to use the configured model.' },
        stability: { type: 'number', minimum: 0, maximum: 1, description: 'ElevenLabs only. Defaults to 0.5.' },
        speed: { type: 'number', minimum: 0.5, maximum: 2, description: 'ElevenLabs (0.7–1.2), MiniMax (0.5–2), OpenAI, Cartesia, or Qwen only. Defaults to 1; provider/model support is validated server-side.' },
        ...
        languageCode: { type: 'string', description: 'ElevenLabs, Cartesia, or Qwen only. Provider/model support is validated server-side.' },
        seed: { type: 'integer', minimum: 0, maximum: 4294967295, description: 'ElevenLabs reproducibility seed.' },
        outputFormat: { type: 'string', description: 'ElevenLabs official output enum (for example mp3_44100_128) or OpenAI/Gemini/Mistral/Cartesia/Qwen provider-supported output format.' },
        instructions: { type: 'string', description: 'OpenAI, Gemini, or Qwen only. Optional natural-language delivery instructions; send only when requested and supported by the configured model.' },
        ...
        stream: { type: 'boolean', description: 'MiniMax HTTP streaming transport. OpenChatCut consumes it server-side and saves one durable audio asset.' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        stream: { type: 'boolean', description: 'Mureka only. Enable provider streaming phase; OpenChatCut still saves durable final files.' },
  ...
  {
    name: 'submit_video',
    description: 'Submit a Seedance 2.0, Kling, MiniMax Hailuo, BytePlus Seedance, xAI Grok Imagine, OFox, or Jimeng digital-human video generation job and create one video asset in the project media pool. Does not place the video on the timeline. Keep image, video, and audio references in their matching arrays. For a recognizable real person, collect explicit likeness consent before submitting.',
    input_schema: {
      type: 'object',
      properties: {
        model: { type: 'string', enum: ['seedance2', 'kling', 'hailuo', 'byteplus', 'grok-imagine-video', 'ofox', 'jimeng-avatar'], description: 'hailuo is MiniMax: 6 or 10s; firstFrame optional; lastFrame allowed with firstFrame; no multi-ref or multi-shot. 1080p is 6s only. byteplus is BytePlus ModelArk Seedance — same request shape/limits as seedance2. grok-imagine-video is xAI Grok Imagine: text-to-video only, 1–15s, audio track included, no references/frames. ofox is the OFox multi-model gateway (Seedance/Wan and more behind one key): 2–30s with per-model limits enforced by the API; supports firstFrame (and optional lastFrame), or up to 9 refImages (frames and refImages are mutually exclusive); no refVideos/refAudios yet. jimeng-avatar is Jimeng OmniHuman quick mode: exactly one firstFrame image plus exactly one refAudios asset; audio must be 15 seconds or less, duration follows the audio, and prompt/duration/ratio are omitted.' }, // minimax: hailuo enum
        prompt: { type: 'string', description: 'Required for normal generation and Kling intelligence; omit for Kling customize.' },
        name: { type: 'string' },
        durationSeconds: { anyOf: [{ type: 'number' }, { type: 'string' }], description: 'Integer seconds, 2–15 for Seedance, 3–15 for Kling, exactly 6 or 10 for Hailuo (Hailuo 1080p → 6 only), 1–15 for grok-imagine-video, 2–30 for ofox (per-model limits enforced by the API). Omit for jimeng-avatar; output duration follows the audio.' }, // minimax: hailuo durations
        ...
        firstFrame: { type: 'string', description: 'Project image asset ID, asset:// ID, short unique ID prefix, or same-project asset path. Required for jimeng-avatar.' },
        ...
        refAudios: { type: 'array', items: { type: 'string' }, description: 'Audio asset references. jimeng-avatar requires exactly one audio asset, which drives the speaking motion and duration.' },
        likenessConsent: { type: 'boolean', description: 'jimeng-avatar only. Set true only after the user explicitly confirms: 我确认拥有该肖像，或已获得创建和使用该数字人形象的授权，并承诺不将其用于冒充他人、欺诈或其他违法用途。' },
  ...
  {
    name: 'track_progress',
    description: 'Inspect or wait for asynchronous generation jobs returned by Sonilo submit_sound, submit_music, and submit_video, including Jimeng digital-human video jobs. Successful results are added to the project media pool exactly once.',
    input_schema: {
      type: 'object',
      properties: {
