// Installed-pack → editor template/zoom mapping, without React or UI imports.
//
// Split out of pluginResources.ts because headless hosts need the very same
// mapping: the offline MCP session and the occ CLI build their template catalog
// from the installed packs on disk, and importing the UI module (which pulls
// ResourceBrowser.tsx) would drag JSX into the node-side build.
