...
import { normalizeSelectedLoudness } from '../audio/normalizeSelectedLoudness';
...
import { flexCropInsetPatch } from './flexCrop';
import { createDigitalHuman } from './digitalHumanFlow';
...
function buildEditorWorkspaceViewProps(
  ...
  const { chatSeed, setChatSeed, showDesign, setShowDesign, showVersions, setShowVersions, showShortcuts, setShowShortcuts, showSettings, setShowSettings, digitalHumanOpenNonce, openDigitalHuman, shortcutApiRef, getPlayhead } = ui.dialogs;
  ...
  const { startAssetTranscription, ingestToPool, importMobileUpload, importToPool, dropExternalFilesToTimeline, addMediaAssetsToTimeline, importToCanvas, pasteMediaAssets, useMediaAI: seedMediaForAI, useCoursewareAI: generateCourseFromCourseware, useTemplateAI } = ui.ingest;
  ...
  return {
    ...
    topBar: { projectName: project.name, exporting: activeExportJobs > 0, exportJobCount: activeExportJobs, canUndo, canRedo, onHome: handleHome, onRename, onSettings: () => setShowSettings(true) },
    ...
    chatPanel: { ctx: agentCtx, projectId: project.id, collapsed: chatCollapsed, onToggleCollapse: () => setChatCollapsed((v) => !v), onPreviewState: setPreviewState, seed: chatSeed, creativeMode, onCreativeModeChange: changeCreativeMode, onImportMedia: importToPool, onOpenDigitalHuman: openDigitalHuman },
    chatCollapsed,
    onResizeChat: panelLayout.resizeChat,
    libraryPanel: { semanticScopeId: project.id, digitalHumanOpenNonce, templates: allTemplates, onAddTemplate: addTemplate, onAddAudio: (a) => commands.addAudio(a), onCreateDigitalHuman: (input, onStage) => createDigitalHuman(input, { projectId: project.id, getState: () => stateRef.current, addAsset: commands.addAsset, onStage }), onGenerateCourse: async (courseAssets, action, digitalHuman, courseOptions) => { await generateCourseFromCourseware(courseAssets, action, digitalHuman, courseOptions); if (action === 'video') showAppToast(t('已将智能制课任务发送给 Agent，请确认讲稿后继续生成课程视频。')); }, playerRef, fps: state.fps, items: state.items, trackOptions, captionTracks, onSetCaptions: commands.setCaptions, onUpdateCaptions: commands.updateCaptions, onSetItemTranscript: commands.setItemTranscript, onToggleWord: commands.toggleWord, onCleanScript: commands.cleanScript, onSetGapCap: commands.setGapCap, onSetTranscriptPlayOrder: commands.setTranscriptPlayOrder, onReorderTrackItems: commands.reorderTrackItems, onClearEdits: commands.clearEdits, assets: state.assets ?? [], mediaFolders: doc.mediaFolders, usedAssetIds, offlineAssetIds, onAssetLoadError: (asset) => markMediaOffline(asset.src), onImportMedia: importToPool, onImportMobileMedia: importMobileUpload, onIngestDirectoryAsset: ingestToPool, onTranscribeAsset: startAssetTranscription, onAddMediaItem: (asset) => commands.addMediaItem(asset), onAddMediaAssetsToTimeline: addMediaAssetsToTimeline, onUseMediaAI: seedMediaForAI, onPasteMediaAssets: pasteMediaAssets, onCreateMediaFolder: commands.createMediaFolder, onRenameMediaFolder: commands.renameMediaFolder, onDeleteMediaFolder: commands.deleteMediaFolder, onMoveMediaAssets: commands.moveMediaAssets, onRenameMediaAsset: commands.renameMediaAsset, onRenameMediaAssets: commands.renameMediaAssets, onSetMediaAssetFavorite: commands.setMediaAssetFavorite, onSetMediaAssetsFavorite: commands.setMediaAssetsFavorite, onRemoveMediaAsset: commands.removeMediaAsset, onRemoveMediaAssets: commands.removeMediaAssets, onCreateCaptionTrack: commands.createCaptionTrack, getSequenceOptions, onAddSequence: (timelineId) => { const result = commands.addSequence(timelineId, { startFrame: getPlayhead() }); if (!result.ok) showAppToast(t(result.error), { error: true }); }, onRelinkMediaAsset: relinkMediaAsset, onAddSolid: () => commands.addSolidItem({ startFrame: getPlayhead() }), creativeMode, onCreativeModeChange: changeCreativeMode, onUseTemplateAI: useTemplateAI, selectedItem, onApplyTransition: (type, custom) => state.selectedId && commands.addTransition(state.selectedId, type, undefined, custom), onApplyFx: (assetId) => { if (!state.selectedId) return; const it = state.items.find((x) => x.id === state.selectedId); if (!it) return; const prev = it.effects ?? []; const next = [...prev.filter((e) => e.assetId !== assetId), { id: `fx_${assetId}`, assetId, overrides: {} }]; commands.setItemEffects(state.selectedId, next, serializableDefsFor(next)); }, onApplyZoom: (zoom) => state.selectedId && commands.setItemZoom(state.selectedId, zoom) },
